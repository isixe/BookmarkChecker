...
export function exportToXLSX(results: BookmarkResult[], fileName: string, lang: 'zh' | 'en') {
...
export function exportToCsv(results: BookmarkResult[], fileName: string, lang: 'zh' | 'en') {
  const t = translations[lang];
  const dataToExport = results.map((r, index) => ({
    [t.index]: index + 1,
    [t.title]: r.title,
...
export function exportToJson(results: BookmarkResult[], fileName: string) {
  ...
  const data = new Blob([jsonString], { type: 'application/json;charset=utf-8' });
